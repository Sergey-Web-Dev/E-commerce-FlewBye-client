...
import AdminRemoveItem from "./admin-remove-item";
import AdminPatchItem from "./admin-patch-item";
...
const BlockAdminItems: FC<BlockAdminItems> = ({
  id,
  description,
  img,
  name,
    ...
    <div className="flex flex-col justify-between items-center w-1/4">
      <div className="relative">
        ...
        <button className="absolute top-10 right-5">
          <AdminRemoveItem id={id} />
        </button>

        <button className="absolute top-10 right-24">
          <AdminPatchItem id={id} />
        </button>
      </div>
      <div>
        <div>{name}</div>
...
export default BlockAdminItems;
